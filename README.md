...
- **Marketing Landing Page & Web App Routing Split (`/` vs `/app`)**:
  ...
  - **SERP SEO & AI Discovery Infrastructure**: 6-entity Schema.org JSON-LD linked graph (`Organization`, `Person`, `WebSite`, `WebApplication`, `BreadcrumbList`, `FAQPage`) with founder attribution to Madhu Dadi (`parentOrganization` and `founder`), strict-budget meta tags, zero external CDN dependencies, and standard machine discovery endpoints (`/robots.txt`, `/sitemap.xml`, `/llms.txt`, `/llms-full.txt`, `/ai-profile.json`) with crawler cookie isolation and RFC 9309 scraper policies.
